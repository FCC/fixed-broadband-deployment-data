/**************************************/
/* CODE FOR MOCKUP ONLY -- DO NOT USE */
/**************************************/
...
function createMap() {
    ...
    bpr_county_layer_fixed_1 = L.tileLayer.wms(geo_host + '/gwc/service/wms?tiled=true', {
        format: 'image/png',
        transparent: true,
    ...
    bpr_county_layer_fixed_0 = L.tileLayer.wms(geo_host + '/gwc/service/wms?tiled=true', {
        format: 'image/png',
        transparent: true,
    ...
    bpr_county_layer_urban = L.tileLayer.wms(geo_host + '/wms', {
        format: 'image/png',
        transparent: true,
    ...
    bpr_tribal = L.tileLayer.wms(geo_host + '/wms', {
        format: 'image/png',
        transparent: true,
...
function fetchCounty(lat, lng) {
    ...
    var url = geo_host + "/" + geo_space + "/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + geo_space + ":bpr_county&maxFeatures=1&outputFormat=application/json&cql_filter=contains(geom,%20POINT(" + lng + " " + lat + "))";
...
function fetchBlock(lat, lng) {
    ...
    var url = geo_host + "/" + geo_space + "/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + geo_space + ":bpr_block_layer&maxFeatures=1&outputFormat=application/json&cql_filter=contains(geom,%20POINT(" + lng + " " + lat + "))" + "&format_options=callback:parseResponse";    
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            var url = geo_host + "/" + geo_space + "/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + geo_space + ":bpr_block_info&maxFeatures=100&outputFormat=text/javascript&cql_filter=block_fips='" + block_fips + "'";
            var url = geo_host + "/" + geo_space + "/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + geo_space + ":bpr_block_info&maxFeatures=100&outputFormat=application/json&cql_filter=block_fips='" + block_fips + "'";            
...
function sort_dbaname_0(a, b) {
    if (a.dbaname == b.dbaname) {
        if (a.technology < b.technology) {
            return -1;
        } else if (a.technology > b.technology) {
            return 1;
        } else {
            return 0;
        }
...
function sort_technology_0(a, b) {
    if (a.technology == b.technology) {
        if (a.download_speed < b.download_speed) {
            return -1;
        } else if (a.download_speed > b.download_speed) {
            return 1;
        } else {
            return 0;
        }
...
function sort_download_0(a, b) {
    if (a.download_speed == b.download_speed) {
        if (a.upload_speed < b.upload_speed) {
            return -1;
        } else if (a.upload_speed > b.upload_speed) {
            return 1;
        } else {
            return 0;
        }
